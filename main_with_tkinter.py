...
def play_round(guess, data, nlp, output_box):
    ...
    if guess in data["already_guessed"]:
        ...
        if guess == secret_word.lower():
            ...
            tk_print(f"Number of Hints: {round_hints}", output_box) 
...
def letter_reveal(data, output_box): 
    ...
    if given >= len(data["sw"]) - 2:
        tk_print("You've used up all the letter reveal hints for this round", output_box)
...
if __name__ == "__main__":  
    ...
    output_box.configure(state="disabled")
    output_box.configure(tabs=("60p", "250p"))
